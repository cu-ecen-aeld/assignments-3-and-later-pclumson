...
void* threadfunc(void* thread_param)
{
    ...
    if ( rc != 0)
    {
        ERROR_LOG("pthread_mutex_lock failes with %d\n", rc);
        return thread_param;
    }

    usleap(thread_data->wait_to_release_ms * 1000);

    rc = pthread_mutex_unlock(thread_data->pmutex);
    if ( rc != 0)
    {
        ERROR_LOG("pthread_mutex_lunock failes with %d\n", rc);
        return thread_param;
    }
...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    pdata->pmutex = mutex;
    pdata->wait_to_release_ms = wait_to_obtain_ms;
